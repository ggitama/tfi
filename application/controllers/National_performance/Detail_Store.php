...
class Detail_Store extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($get_data_iframe) {
            ...
            $data['iframe_tag'] = $template_iframe->tag_open . $template_iframe->ip_iframe . $iframe_tag . "'" . $template_iframe->width . "height='9000'" . $template_iframe->attributes . $template_iframe->tag_close;
        } else {
            $data['iframe_name'] = '';
        ...
        $data['menus'] = $menu;
        $data['title'] = 'DETAIL STORE';
        $data['menu_header'] = 'Dashboard';
        $data['main_menu'] = 'DETAIL STORE';

        $this->load->view('template_dashboard/Header_v', $data);
        $this->load->view('National_performance/Detail_Store_v', $data);
