<div class="body flex-grow-1 p-4 ms-5 me-5" style="background-color: #ffff;">
    ...
    <div class="row">
        <div class="col-md-4">
            <button class="btn btn-sm btn-primary" value="" data_modal="modal_add" onclick="modal_view()"> ADD ROLE</button>
        </div>
    </div>
    <h2 class="text-center">Role Menu</h2>
    <table id="roleManage" class="table table-bordered table-striped text-center align-middle" width="100%">
...
<script>
    ...
    function default_dt() {
        $('#roleManage').DataTable({
            ...
            "ajax": {
                "url": "<?= htmlentities(base_url('Parameter/Manage_Role_Menu_c/view_query/')) ?>", // URL file untuk proses select datanya
                "type": "POST",
                "data": function(d) {
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        if (data.role_name != 'Admin') {
                            return `<button class="btn btn-success m-2" data_modal="modal_edit" onclick="edit_modal()" value="${data.role_id}">Edit</button>
                                <button class="btn btn-danger ms-3" onclick="delete_modal()" value="${data.role_id}">Delete</button>`;
                        } else {
                            return `<button class="btn btn-success m-2" data_modal="modal_edit" onclick="edit_modal()" value="${data.role_id}">Edit</button>`;
    ...
    function modal_view() {
        ...
        val.id = 'modal_add';
        $.ajax({
            url: '<?= htmlentities(base_url('Parameter/Manage_Role_Menu_c/modal_add/')) ?>',
            type: "post",
            data: val,
            beforeSend: function() {
                $("#loader").show();
    ...
    function edit_modal() {
        ...
        $.ajax({
            url: '<?= htmlentities(base_url('Parameter/Manage_Role_Menu_c/modal/')) ?>',
            type: "post",
            data: val,
            beforeSend: function() {
                $("#loader").show();
    ...
    function delete_modal() {
        ...
        val.id = 'modal_delete';
        val.role_id = event.target.value;
        $.ajax({
            url: '<?= htmlentities(base_url('Parameter/Manage_Role_Menu_c/modal_delete/')) ?>',
            type: "post",
            data: val,
            beforeSend: function() {
                $("#loader").show();
    ...
    function add_role_user(data_) {
        ...
        if (action == 'modal_delete') {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo htmlentities(site_url('Parameter/Manage_Role_Menu_c/validate/')) ?>",
                data: form,
                dataType: "json",
                beforeSend: function() {
                    $("#loader").show();
    ...
    function delete_(form) {
        form = $("#task2").serialize() + '&<?= $this->security->get_csrf_token_name(); ?>=' + val.<?= $this->security->get_csrf_token_name(); ?>;
        Swal.fire({
            title: 'Are you sure?',
    ...
    function save() {
        form = $("#task").serialize() + '&<?= $this->security->get_csrf_token_name(); ?>=' + val.<?= $this->security->get_csrf_token_name(); ?>;
        Swal.fire({
    ...
    function key(tes) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo htmlentities(site_url('Parameter/Manage_Role_Menu_c/validate_keyup/')) ?>",
            data: val,
            dataType: "json",
            success: function(data) {
                val.<?= htmlentities($this->security->get_csrf_token_name()); ?> = data.token;
