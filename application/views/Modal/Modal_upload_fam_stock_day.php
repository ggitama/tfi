    ...
    <div class="modal-content">
      <form id="form_<?= $id ?>" method="post" enctype='multipart/form-data'>
        <div class="modal-header">
          ...
          <a href="<?= base_url() ?>Parameter/Stock_Days_c0001/export_to_excel_fam?store=<?=$store?>&biu=<?=$biu?>&dept=<?=$dept?>&cat=<?=$cat?>&fam=<?=$fam?>" target="_blank">
          <button type="button" class="btn btn-success ms-5">Template Excel Stock Days Family</button>
        </a>
        ...
        <div class="modal-body">
          ...
          <div class="mb-3 row">
            <div class="col-sm-7 form-group">
              <input type="file" id="input-media_file" name="files[]'" class="form-control">
            </div>
...
<script>
  ...
  function upload_media(datas) {
    ...
    var file_data = $('#input-media_file').prop('files')[0];
    ...
    if (typeof file_data == 'undefined') {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: '<?= base_url('Parameter/Stock_Days_c0001/upload_fam_stockdays/') ?>', // <-- point to server-side PHP script 
            ...
            //   $('#modal_upload_fam_stock_day').modal('toggle');
              ...
              Swal.fire({
                ...
                $("#modal_upload_fam_stock_day").modal('hide');
                $("#modal_upload_fam_stock_day").remove();
                // location.reload();
