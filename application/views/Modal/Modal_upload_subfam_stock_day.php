    ...
    <div class="modal-content">
      <form id="form_<?= $id ?>" method="post" enctype='multipart/form-data'>
        <div class="modal-header">
          ...
          <a href="<?= base_url() ?>Parameter/Stock_Days_c0001/export_to_excel_subfam?store=<?=$store?>&biu=<?=$biu?>&dept=<?=$dept?>&cat=<?=$cat?>&fam=<?=$fam?>&subfam=<?=$subfam?>" target="_blank">
          <button type="button" class="btn btn-success ms-5">Template Excel Stock Days Sub Family</button>
        </a>
        ...
        <div class="modal-body">
          ...
          <div class="mb-3 row">
            <div class="col-sm-7 form-group">
              <input type="file" id="input-media_filesubfam" name="files[]'" class="form-control">
            </div>
...
<script>
  ...
  function upload_media(datas) {
    ...
    var file_data = $('#input-media_filesubfam').prop('files')[0];
    ...
    if (typeof file_data == 'undefined') {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: '<?= base_url('Parameter/Stock_Days_c0001/upload_subfam_stockdays/') ?>', // <-- point to server-side PHP script 
