        ...
        <div class="modal-content">
            <form id="form_<?= htmlentities($id) ?>">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-7 form-group">
                            ...
                            <div id="error"></div>
                            <input type="hidden" id="famcode" value="<?= $fams->famcode?>">
                        </div>
                    </div>
                    <div class="mb-3 row">
                        <label class="col-sm-5 col-form-label">Stock Day Min</label>
                        <div class="col-sm-7 form-group">
                            <input type="number" id="edit-sdMin" value="<?= $stock_day_min?>" name="sdMin" class="form-control" onkeypress="if ( isNaN(this.value + String.fromCharCode(event.keyCode) )) return false;" disabled>
                            <div id="error"></div>
                        </div>
                    </div>
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-7 form-group">
                            <input type="number" id="edit-sdMax" value="<?= $stock_day_max?>" name="sdMax" class="form-control" onkeypress="if ( isNaN(this.value + String.fromCharCode(event.keyCode) )) return false;" disabled>
                            <div id="error"></div>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" data="<?= htmlentities($id) ?>" onclick="deleteStockDaysFam(this)" class="btn btn-danger">DELETE</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    function deleteStockDaysFam(data) {
        ...
        if (validated.validate) {
            ...
            $.ajax({
                url: '<?= htmlentities(base_url('Parameter/Stock_Days_c0001/delete_sdFam/')) ?>',
                type: "post",
                data: val,
                beforeSend: function() {
                    $("#loader").show();
                ...
                success: function(res) {
                    ...
                    if (response.value == '0') {
                        ...
                        Swal.fire({
                            ...
                            // Reload the Page
                            // location.reload();
                            $("#delete_modal_fam_stock_day").modal('hide');
                            $("#delete_modal_fam_stock_day").remove();
